...
// When set, pickerSave replaces config.selectors[_pickerEditIndex] instead
// of appending. Used by the popup's edit/re-pick action.
let _pickerEditIndex = null;
 ...
 */
function pickerActivate(hostname, editIndex = null) {
  _pickerState = PICKER_STATE.HOVERING;
  _pickerHostname = hostname;
  _pickerEditIndex = (typeof editIndex === 'number' && editIndex >= 0) ? editIndex : null;
  injectPickerBanner();
...
async function pickerSave(selectorString, label) {
  ...
  // If editing, preserve the existing enabled/forceRTL flags so the user's
  // per-selector toggles survive a re-pick. Otherwise, default to enabled+force.
  const existing = (_pickerEditIndex !== null)
    ? config.selectors[_pickerEditIndex]
    : null;
  const entry = {
    selector: selectorString,
    enabled: existing?.enabled ?? true,
    forceRTL: existing?.forceRTL ?? true,
  };
  if (label) entry.label = label;
  if (_pickerEditIndex !== null && _pickerEditIndex < config.selectors.length) {
    config.selectors[_pickerEditIndex] = entry;
  } else {
